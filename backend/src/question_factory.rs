...
impl QuestionFactory {
    ...
    fn get_constellations_pool(difficulty: &str) -> Vec<String> {
    ...
    fn get_magnitude_limit(difficulty: &str) -> f32 {
    ...
    pub fn make_question(
        ...
        match mode {
            "constellation" => {
                ...
                let tilt_angle = if random_tilt { Some(rng.gen_range(0.0..360.0)) } else { Some(0.0) };

                let config = RenderConfig {
                    projection: "pinhole".to_string(),
                    datetime: None,
                    latitude: 45.0, // дефолты для снимка
                    longitude: 19.0,
            ...
            "star" => {
                let mag_limit = Self::get_magnitude_limit(difficulty);
                // Фильтруем звезды из каталога
                let stars = hip_catalog.get_stars(mag_limit, None);
                let star_ids: HashSet<i32> = stars.iter().map(|s| s.hip_id).collect();

                // Ищем доступные именные звезды
                let available_stars: Vec<(i32, String)> = NAMED_STARS.iter()
                ...
                if pool.is_empty() {
                    used_objects.clear();
                    pool = NAMED_STARS.iter()
                        .filter(|&&(hip, _)| star_ids.contains(&hip))
                        .map(|&(hip, name)| (hip, name.to_string()))
                        .collect();
                }

                // Запасной вариант, если каталог пуст
                if pool.is_empty() {
                    pool = NAMED_STARS.iter().map(|&(hip, name)| (hip, name.to_string())).collect();
                }

                let (correct_hip, correct_name_en) = pool.choose(&mut rng).cloned().unwrap_or((32349, "Sirius".to_string()));
                ...
                // Опции для Easy режима
                let mut other_names: Vec<String> = NAMED_STARS.iter()
            ...
            "messier" => {
                ...
                let mut available: Vec<MessierObject> = all_objects.iter()
                ...
                // Рендерим снимок с подсветкой Мессье
                let config = RenderConfig {
                    projection: "pinhole".to_string(),
                    datetime: None,
                    latitude: 45.0,
                ...
                let mut distractors = all_objects.iter()
                    .filter(|o| o.m_number != m_num)
                    .collect::<Vec<_>>();
                distractors.shuffle(&mut rng);
                
                let mut selected_distractors = vec![];
                // 1. Попытка взять из того же созвездия
                let mut same_const: Vec<_> = distractors.iter()
                ...
                let fact = MESSIER_FACTS.iter()
                    .find(|&&(num, _)| num == m_num)
                    .map(|&(_, f)| f.to_string())
                    .unwrap_or_else(|| format!("M{} — {} в созвездии {}.", m_num, ru_type, correct_obj.constellation));

                let question_text = format!("Объект типа «{}» в созвездии {}. Что это за объект Мессье?", ru_type, correct_obj.constellation);
